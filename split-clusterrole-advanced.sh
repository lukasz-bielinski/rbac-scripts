...
Examples:
  ...
  $0 engineer-role.yaml --output-dir ./my-roles --group-by apigroup
EOF
    exit 1
fi
...
### Apply all roles directly:
\`\`\`bash
kubectl apply -f $OUTPUT_DIR/
\`\`\`

### Apply in order (alternative):
\`\`\`bash
...
# Kustomization for $ORIGINAL_ROLE_NAME
# Generated by split-clusterrole-advanced.sh
# $(date)
...
resources:
  ...
  - 00-AGGREGATOR.${OUTPUT_FORMAT}
  
  # Component roles (automatically aggregated)
EOF
    
    # Add all component files (sorted, excluding aggregator and kustomization)
    for file in $(ls -1 "$OUTPUT_DIR"/*."$OUTPUT_FORMAT" 2>/dev/null | sort); do
        FILENAME=$(basename "$file")
        [[ "$FILENAME" == "00-AGGREGATOR."* ]] && continue
        [[ "$FILENAME" == "kustomization."* ]] && continue
        echo "  - ${FILENAME}" >> "$KUSTOMIZATION_FILE"
    done
    
    log_success "Created kustomization: $KUSTOMIZATION_FILE"
fi
